<template>
	<div class="py-5 space-y-8">
		...
		══════════════════════════════════════════════════════════════ -->
		<header class="pb-4 border-b-2 border-[#003399]/12 full-bleed px-4 sm:px-6 lg:px-8">
			...
			<div class="flex items-center justify-between gap-3 sm:hidden">
				...
				<div class="flex flex-col gap-2 items-end shrink-0">
					...
					<LangSwitcher :full="true" />
				</div>
			</div>

			<!-- Titre (mobile uniquement, sous la ligne logo+boutons) -->
			<div class="mt-3 text-center sm:hidden">
				<h1 class="text-2xl font-normal font-title tracking-tight text-slate-900 leading-tight animate-fade-in-down">
					RCAM / Article 72<span class="text-lg">§</span>3
				</h1>
			</div>

			<!-- Desktop : [logo] [titre centré absolu] [boutons] -->
			<div class="hidden sm:flex sm:items-center sm:justify-between sm:gap-4 relative">
				<!-- Logo -->
				<img
					:src="'/European-Union-Flag-256.png'"
					alt="Commission Européenne"
					class="h-20 lg:h-24 object-contain shrink-0"
				>

				<!-- Titre centré sur toute la largeur de la page -->
				<div class="absolute inset-x-0 text-center pointer-events-none">
					<h1 class="text-3xl lg:text-4xl font-normal font-title tracking-tight text-slate-900 leading-tight animate-fade-in-down">
						RCAM / Article 72<span class="text-xl lg:text-2xl">§</span>3
					</h1>
					<p class="text-base text-slate-500 mt-1 animate-fade-in-up">
						{{ t('app.subtitle') }}
					</p>
				</div>

				<!-- Boutons (thème + licence + profil) avec langue en dessous -->
				<div class="flex flex-col gap-2 items-end shrink-0 relative z-10">
					<div class="flex items-center gap-2">
						<button
		...
		<template v-else>
			...
			<div class="text-center py-10">
				<div v-if="meilleureFenetre && meilleureFenetre.depassement > 0" class="inline-block animate-bounce-in relative">
					<!-- 12 étoiles orbitant sur chemin elliptique (inset: -30px autour de la carte) -->
					<div class="absolute pointer-events-none" style="inset: -30px">
						<span
							v-for="n in 12"
							:key="n"
							class="text-2xl leading-none shimmer-blue-gold"
							:style="{
								position: 'absolute',
								offsetPath: 'ellipse(50% 50% at 50% 50%)',
								offsetRotate: '0deg',
								offsetAnchor: '50% 50%',
								animation: `orbit-cw 15s linear ${(-((n - 1) * 15 / 12)).toFixed(2)}s infinite, blink-star-orbit 1.5s ease-in-out ${((n - 1) * 0.125).toFixed(2)}s infinite`
							}"
						>★</span>
					</div>
					<!-- Carte (devant les étoiles) -->
					<div class="relative z-10 rounded-tr-2xl rounded-bl-2xl rounded-tl-none rounded-br-none px-8 py-5">
						<div class="flex items-center justify-center gap-2 text-xs font-black uppercase tracking-widest mb-3">
							<UIcon name="heroicons:star-solid" class="size-4 blink-star text-[#003399] dark:text-[#FFCC00]" />
							<span class="shimmer-blue-gold" style="animation: shimmer-ltr 28s linear infinite;">{{ t('banner.best_period') }}</span>
							<UIcon name="heroicons:star-solid" class="size-4 blink-star text-[#003399] dark:text-[#FFCC00]" />
						</div>
						<div class="text-2xl lg:text-3xl font-black tracking-wide shimmer-blue-gold">
			...
			<div class="bg-white rounded-2xl border-2 border-slate-200 p-4 shadow-sm">
				...
				<div class="flex items-center justify-between mb-3">
					<div class="flex items-center gap-1.5">
						<UIcon name="lucide:zap" class="size-3.5 text-[#FFCC00] shrink-0" />
						<span class="text-xs font-black uppercase tracking-wider bg-gradient-to-r from-[#003399] to-[#FFCC00] bg-clip-text text-transparent">{{ t('quick.title') }}</span>
					</div>
					<div v-if="salaireNet > 0" class="flex items-center gap-2">
						...
						<strong v-if="showSeuil" class="text-sm font-black text-amber-500 tabular-nums whitespace-nowrap">{{ fmt(salaireNet / 2) }} €</strong>
				...
				<div>
					...
					<div v-else class="space-y-3">
						<!-- Tous les champs sur une ligne, wrap naturel sur petits écrans -->
						<div class="flex flex-wrap items-end gap-3">
							...
							<div class="w-24 shrink-0">
								...
								<input
									...
									placeholder="xxx"
									class="w-full h-12 px-3 text-base text-center rounded-xl border-2 border-slate-200 focus:border-[#003399] focus:ring-2 focus:ring-[#003399]/15 outline-none transition-all bg-white text-slate-800"
								>
							</div>
							...
							<div class="shrink-0">
								...
								<input
									...
									type="date"
									class="h-12 px-3 text-base rounded-xl border-2 border-slate-200 focus:border-[#003399] focus:ring-2 focus:ring-[#003399]/15 outline-none transition-all bg-white text-slate-800"
								>
							</div>
							...
							<div class="shrink-0">
								...
								<div class="flex items-center gap-2">
									<input
										...
										min="0"
										placeholder="0.00"
										class="w-32 h-12 px-3 text-base text-center rounded-xl border-2 border-slate-200 focus:border-[#003399] focus:ring-2 focus:ring-[#003399]/15 outline-none transition-all bg-white text-slate-800"
									>
									<span class="text-lg font-bold text-amber-500 shrink-0 dark:text-[#FFCC00]">€</span>
								</div>
							</div>
						</div>
						...
						<div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
							...
							<div v-if="meilleureFenetre && meilleureFenetre.totalReste > 0" class="flex items-center gap-3 px-4 py-2 rounded-xl bg-blue-50 border border-[#003399]/20 self-start sm:self-auto">
								...
								<span class="text-2xl font-black text-amber-500 tabular-nums dark:text-[#FFCC00]">{{ fmt(meilleureFenetre.totalReste) }} €</span>
							</div>
			...
			<div v-if="meilleureFenetre && meilleureFenetre.depassement > 0" class="flex flex-col lg:flex-row gap-6">
				...
				<div class="flex-1 bg-white rounded-2xl border-2 border-slate-200 overflow-hidden shadow-sm">
					<div class="px-5 py-3 bg-slate-50 border-b-2 border-slate-100 flex items-center gap-2">
						<UIcon name="lucide:files" class="size-4 text-[#FFCC00] shrink-0" />
						<span class="text-xs font-black uppercase tracking-wider bg-gradient-to-r from-[#003399] to-[#FFCC00] bg-clip-text text-transparent">{{ t('period.title') }}</span>
						...
						>
							<div class="min-w-0">
								...
								<div class="text-sm text-slate-400 flex gap-2 flex-wrap mt-0.5">
									<span v-if="f.numeroPage" class="dark:text-[#FFCC00]">({{ f.numeroPage }})</span>
									<span v-if="f.pays">· {{ f.pays }}</span>
								</div>
							</div>
							<div class="text-right shrink-0">
								<div class="text-lg font-black text-amber-500 tabular-nums dark:text-[#FFCC00]">{{ fmt(f.reste) }} €</div>
							</div>
						</div>
				...
				<div class="shrink-0 lg:w-88 space-y-4">
					...
					<div
						...
						:class="meilleureFenetre.remboursementCommission > 0
							? 'bg-[#EBF0FF] border-[#003399]/30 card-ce-alert'
							: 'bg-slate-50 border-slate-200'"
					>
						<div class="text-xs font-black uppercase tracking-wide whitespace-nowrap mb-3" :class="meilleureFenetre.remboursementCommission > 0 ? 'text-[#003399]' : 'text-slate-400'">
							{{ t('ce.title', { taux: personneACharge === 1 ? '100 %' : '90 %' }) }}
						</div>
						<div
							class="font-black leading-none tabular-nums whitespace-nowrap dark:text-[#FFCC00]"
						...
						<div v-if="meilleureFenetre.remboursementCommission > 0" class="mt-3 flex items-center justify-center gap-1 text-sm font-bold text-[#003399]">
					...
					<button
						v-if="isPremium"
						class="btn-submit-gradient w-full flex items-center justify-center gap-3 py-5 rounded-2xl font-black text-lg transition-opacity"
					...
					>
						...
						<div class="min-w-0">
							...
							<div class="flex gap-3 mt-1 flex-wrap text-sm">
								<span v-if="f.numeroPage" class="text-sky-600 font-semibold dark:text-[#FFCC00]">({{ f.numeroPage }})</span>
								<span v-if="montantJSISEffectif(f) > 0" class="text-slate-500">
									JSIS <strong class="text-amber-500 tabular-nums dark:text-[#FFCC00]">{{ fmt(montantJSISEffectif(f)) }} €</strong>
								</span>
							</div>
						</div>
						...
						<div class="flex items-center justify-end gap-1">
							<div class="text-lg font-black text-right mr-1 shrink-0 tabular-nums dark:text-[#FFCC00]" :class="f.soumis ? 'text-amber-400' : 'text-amber-500'">
								{{ fmt(resteFactureAffichage(f)) }} €
							</div>
							<button
								v-if="!f.soumis"
								class="size-9 rounded-xl hover:bg-orange-50 flex items-center justify-center text-orange-500 hover:text-orange-600 transition-colors"
								:title="t('all.edit')"
							...
							<button
								class="size-9 rounded-xl hover:bg-red-50 flex items-center justify-center text-red-500 hover:text-red-700 transition-colors"
								:title="t('all.delete')"
				...
				<div v-else class="space-y-6">
					<div v-for="groupe in groupesSoumises" :key="groupe.id" class="rounded-2xl border border-slate-200 overflow-hidden">
						...
						<div class="flex items-center gap-3 px-5 py-4 bg-slate-100 flex-wrap">
							...
							<span class="text-base font-bold ml-auto tabular-nums text-amber-500 dark:text-[#FFCC00]">{{ fmt(groupe.total) }} €</span>
							<button
								class="btn-cancel-lot flex items-center gap-2 px-3 py-2 rounded-xl border-2 font-semibold text-sm"
							...
							>
								<div class="flex-1 min-w-0">
									...
									<div class="flex gap-3 mt-0.5 text-sm">
										<span v-if="f.numeroPage" class="text-slate-400 font-semibold dark:text-[#FFCC00]">({{ f.numeroPage }})</span>
									</div>
								</div>
								<div class="text-right shrink-0">
									<div class="text-base font-semibold text-amber-400 tabular-nums dark:text-[#FFCC00]">{{ fmt(resteFactureAffichage(f)) }} €</div>
								</div>
								<button
									class="size-9 rounded-xl hover:bg-red-50 flex items-center justify-center text-red-500 hover:text-red-700 transition-colors"
									@click="confirmDelete(f)"
							...
							<div class="rounded-2xl bg-amber-50 border-2 border-amber-200 p-5">
								...
								<div class="space-y-1.5">
									<div v-for="d in doublonsDetectes" :key="d.id" class="text-sm text-slate-600 bg-white rounded-xl px-3 py-2 border border-slate-200">
										{{ d.prenom }} {{ d.nom }} — {{ formatDate(d.dateFacture) }} — <span class="text-amber-500 tabular-nums dark:text-[#FFCC00]">{{ fmt(montantTotalEuroEffectif(d)) }} €</span><span v-if="d.numeroPage"> — P.<span class="dark:text-[#FFCC00]">{{ d.numeroPage }}</span></span>
									</div>
								</div>
							</div>
					...
					<div class="modal-card relative bg-white rounded-3xl shadow-2xl overflow-hidden w-full max-w-lg">
						...
						<div class="px-7 py-6 space-y-5">
							...
							<div v-if="(meilleureFenetre?.remboursementCommission ?? 0) > 0" class="rounded-2xl bg-blue-50 border-2 border-[#003399]/20 p-5 text-center">
								...
								<p class="text-4xl font-black text-amber-500 dark:text-[#FFCC00]">{{ fmt(meilleureFenetre?.remboursementCommission ?? 0) }} €</p>
							</div>
							<div class="flex gap-3">
						...
						<div v-if="factureEditee" class="px-7 py-6 space-y-5">
							...
							<div>
								...
								<div class="flex items-center gap-3">
									...
									<span class="text-xl font-bold text-slate-400 shrink-0 dark:text-[#FFCC00]">€</span>
								</div>
							</div>
...
const confirmerSoumissionComplete = async () => {
	...
	try {
		...
		if (data) {
			pdfBytes = await fillFormRemb(data, currentLocale.value);
		...
		if (os === "macos") {
			...
			const wrapB64 = (b64: string) => b64.match(/.{1,76}/g)?.join('\r\n') ?? b64;
			const CRLF = '\r\n';
			const boundary = `----=_RcamBoundary_${Date.now()}`;
			const subjB64 = toB64(new TextEncoder().encode(sujet));
			const bodyText = corpsLignes.join('\r\n');
			const bodyB64 = wrapB64(toB64(new TextEncoder().encode(bodyText)));
			let eml = '';
			eml += `MIME-Version: 1.0${CRLF}`;
			eml += `X-Unsent: 1${CRLF}`;
			eml += `To: PMO-RCAM@ec.europa.eu${CRLF}`;
			eml += `Subject: =?UTF-8?B?${subjB64}?=${CRLF}`;
			if (pdfBytes) {
				const pdfB64 = wrapB64(toB64(pdfBytes));
				const pdfName = pdfPath.split(/[\\/]/).pop()!;
				eml += `Content-Type: multipart/mixed; boundary="${boundary}"${CRLF}`;
				eml += CRLF;
				eml += `--${boundary}${CRLF}`;
				...
				eml += CRLF;
				eml += `--${boundary}${CRLF}`;
				eml += `Content-Type: application/pdf; name="${pdfName}"${CRLF}`;
				eml += `Content-Transfer-Encoding: base64${CRLF}`;
				eml += `Content-Disposition: attachment; filename="${pdfName}"${CRLF}`;
				eml += CRLF;
				eml += `${pdfB64}${CRLF}`;
				eml += CRLF;
				eml += `--${boundary}--${CRLF}`;
			...
			const emlFilename = `rcam_mail_${dateStr}.eml`;
			await saveFile(emlFilename, new TextEncoder().encode(eml));
			const emlPath = `${rcamAbs}\\${emlFilename}`;
			const emlPathPs = emlPath.replace(/"/g, '`"');
			const psOpen = [
				`$eml = "${emlPathPs}"`,
				'$olk = Get-Command olk.exe -ErrorAction SilentlyContinue',
				'if ($olk) { & $olk.Source $eml } else { Start-Process $eml }',
			].join('\r\n');
			const psOpenFilename = `rcam_open_${dateStr}.ps1`;
			await saveText(psOpenFilename, psOpen);
			const psOpenPath = `${rcamAbs}\\${psOpenFilename}`;
			Command.create("exec-ps", ["-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-File", psOpenPath]).execute()
				.finally(() => invoke("remove_file_abs", { path: psOpenPath }).catch(() => {}))
				.catch(() => {});
		}
		// 4. Marquer soumis
		...
		await load();
		toast.add({ title: "Mail prêt !", description: `Vérifiez le brouillon ouvert${pdfBytes ? ' (PDF joint)' : ''} et cliquez sur Envoyer.`, color: "success" });
	} catch (e) {
		toast.add({ title: t("toast.error"), description: String(e), color: "error" });
	} finally {
